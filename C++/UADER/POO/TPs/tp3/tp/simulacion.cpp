...
void Simulacion::accionPoblacion()
{
    for(int i=0;i<cantPoblaciones;i++){
        for(int j=i+1;j<cantPoblaciones;j++){

            if(poblaciones[i].getSincomer() != 3 &&
                    poblaciones[j].getSincomer() != 3){

                    if(poblaciones[i].getCoordX() == poblaciones[j].getCoordX() &&
                            poblaciones[i].getCoordY() == poblaciones[j].getCoordY() ){

                        char tipo1=poblaciones[i].getTipo();
                        char tipo2=poblaciones[j].getTipo();

                        if(tipo1 == 'A' && tipo2 == 'A') agregarPoblacion('A');

                        if(tipo1 == 'B' && tipo2 == 'B') poblaciones[i].setsincomer(3);
                }
            }
        }
    }
